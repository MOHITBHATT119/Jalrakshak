...
"""
from app.services.data_service import get_village, get_water_demand_data, get_recharge_data, get_data_note
...
def calculate_water_budget(village_id: str, year: int = 2023) -> dict:
    ...
    if demand_data:
        year_data = [d for d in demand_data if int(d["year"]) == year]
        if not year_data:
            year_data = sorted(demand_data, key=lambda x: int(x["year"]))[-1:]
        latest = year_data[-1]
        domestic = float(latest["domestic_demand_mcm"])
        agricultural = float(latest["agricultural_demand_mcm"])
        industrial = float(latest["industrial_demand_mcm"])
        total_demand = float(latest["total_demand_mcm"])
        available_supply = float(latest["available_supply_mcm"])
        deficit = float(latest["deficit_mcm"])
    else:
        # Estimate from village data
        pop = int(village.get("population", 100000)) if village else 100000
        ag_ha = int(village.get("agricultural_area_ha", 30000)) if village else 30000
        ann_rain = int(village.get("annual_rainfall_mm", 600)) if village else 600
        ...
        recharge_mcm = sum(float(r.get("estimated_recharge_mcm", 0)) for r in recharge_data)
    ...
    recharge_total = sum(float(r.get("estimated_recharge_mcm", 0)) for r in recharge_data)
